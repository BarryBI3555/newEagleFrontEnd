...
<script setup lang="ts">
  ...
  import { PersonalMap } from '@views/dashboard/api'
  ...
  const fetchGroupList = async () => {
    try {
      ...
      const allGroups = await PersonalMap.getGroupList(params)
      const userData = await PersonalMap.getLatestLocations(params)
  ...
  const fetchLatestLocations = async () => {
    try {
      ...
      const data = await PersonalMap.getLatestLocations(params)
      clearOverlays()
  ...
  const startLocationProgressPolling = () => {
    ...
    locationProgressTimer = setInterval(async () => {
      try {
        const res = await PersonalMap.getLocationProgress(params)
  ...
  const loadUserTrack = async (usercode: string) => {
    try {
      ...
      const params: Record<string, any> = {}
      if (selectedDate.value) params.date = selectedDate.value
      const data = await PersonalMap.getUserTrajectory(usercode, params)
