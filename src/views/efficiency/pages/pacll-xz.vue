...
<script setup lang="ts">
  ...
  import { PacllXz } from '@views/efficiency/api'
  ...
  const {
    ...
    core: {
      apiFn: async (params: UseTableParams): Promise<UseTableResult<PacllXzData>> => {
        ...
        const response = await PacllXz.getList(queryParams)
  ...
  const handleRefresh = async () => {
    try {
      const res = await PacllXz.getList({ current: 1, size: 9999 })
  ...
  const handleExportAll = async () => {
    try {
      const res = await PacllXz.getList(tableApiParams.value)
