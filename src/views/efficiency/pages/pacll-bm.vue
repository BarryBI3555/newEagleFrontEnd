...
<script setup lang="ts">
  ...
  import { PacllBm } from '@views/efficiency/api'
  ...
  const {
    ...
    core: {
      apiFn: async (params: UseTableParams): Promise<UseTableResult<PacllBmData>> => {
        ...
        const response = await PacllBm.getList(queryParams)
  ...
  const handleRefresh = async () => {
    try {
      const res = await PacllBm.getList({ current: 1, size: 9999 })
  ...
  const handleExportAll = async () => {
    try {
      const res = await PacllBm.getList(tableApiParams.value)
