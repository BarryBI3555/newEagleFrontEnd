...
<script setup lang="ts">
  ...
  import { TableCurGzlRs } from '@views/efficiency/api'
  ...
  const {
    ...
    core: {
      apiFn: async (params: UseTableParams): Promise<UseTableResult<DailyWorkloadRsData>> => {
        ...
        // 最终接口
        const response = await TableCurGzlRs.getList(queryParams)
  ...
  const handleRefresh = async () => {
    try {
      ...
      const res = await TableCurGzlRs.getList({ size: 9999 })
  ...
  const handleExportAll = async () => {
    try {
      ...
      const res = await TableCurGzlRs.getList(tableApiParams.value)
