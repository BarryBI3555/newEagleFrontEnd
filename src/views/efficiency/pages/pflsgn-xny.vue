...
<script setup lang="ts">
  ...
  import { PflsgnXny } from '@views/efficiency/api'
  ...
  const { data: tableData, loading, error: tableError, pagination, refreshData, handleSizeChange, handleCurrentChange, columns, columnChecks } = useTable({
    core: {
      apiFn: async (params: UseTableParams): Promise<UseTableResult<PflsgnXnyData>> => {
        ...
        const response = await PflsgnXny.getList(queryParams)
  ...
  const handleRefresh = async () => {
    try {
      const res = await PflsgnXny.getList({ current: 1, size: 9999 })
  ...
  const handleExportAll = async () => {
    try {
      const res = await PflsgnXny.getList(tableApiParams.value)
