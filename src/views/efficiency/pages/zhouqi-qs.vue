...
<script setup lang="ts">
  ...
  import { ZhouqiQs } from '@views/efficiency/api'
  ...
  const { data: tableData, loading, error: tableError, pagination, refreshData, handleSizeChange, handleCurrentChange, columns, columnChecks } = useTable({
    core: {
      apiFn: async (params: UseTableParams): Promise<UseTableResult<ZhouqiQsData>> => {
        ...
        const response = await ZhouqiQs.getList(queryParams)
  ...
  const handleRefresh = async () => {
    try {
      const res = await ZhouqiQs.getList({ current: 1, size: 9999 })
  ...
  const handleExportAll = async () => {
    try {
      const res = await ZhouqiQs.getList(tableApiParams.value)
