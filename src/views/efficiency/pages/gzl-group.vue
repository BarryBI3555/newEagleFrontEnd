...
<script setup lang="ts">
  ...
  import { GzlGroup } from '@views/efficiency/api'
  ...
  const {
    ...
    core: {
      apiFn: async (params: UseTableParams): Promise<UseTableResult<DailyWorkloadGroupData>> => {
        ...
        const response = await GzlGroup.getList(queryParams)
  ...
  const handleRefresh = async () => {
    try {
      ...
      const res = await GzlGroup.getList({ current: 1, size: 9999 })
  ...
  const handleExportAll = async () => {
    try {
      ...
      const res = await GzlGroup.getList(tableApiParams.value)
