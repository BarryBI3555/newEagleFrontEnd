...
<script setup lang="ts">
  ...
  import { TableCurGzlBm } from '@views/efficiency/api'
  ...
  const {
    ...
    core: {
      apiFn: async (params: UseTableParams): Promise<UseTableResult<DailyWorkloadBmData>> => {
        ...
        const response = await TableCurGzlBm.getList(queryParams)
  ...
  const handleRefresh = async () => {
    try {
      ...
      const res = await TableCurGzlBm.getList({ current: 1, size: 9999 })
  ...
  const handleExportAll = async () => {
    try {
      ...
      const res = await TableCurGzlBm.getList(tableApiParams.value)
