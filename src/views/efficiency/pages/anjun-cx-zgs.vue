<template>
  <div class="flex flex-col gap-2 pb-3">
    ...
    <ElCard class="flex-1 art-table-card" style="margin-top: 0;padding: 5px;">
      <template #header>
        <div class="flex-cb">
          <h4 class="m-0">案均赔款-支公司（车险）【统计时间：{{ currentMaxTjTime }}】</h4>
...
<script setup lang="ts">
  ...
  import { AnjunCxZgs } from '@views/efficiency/api'

  defineOptions({ name: 'AnjunCxZgsTable' })

  // ==================== 1. 类型定义 ====================
  interface AnjunCxZgsData {
  ...
  const { data: tableData, loading, error: tableError, pagination, refreshData, handleSizeChange, handleCurrentChange, columns, columnChecks } = useTable({
    core: {
      apiFn: async (params: UseTableParams): Promise<UseTableResult<AnjunCxZgsData>> => {
        ...
        const response = await AnjunCxZgs.getList(queryParams)
  ...
  const handleRefresh = async () => {
    try {
      const res = await AnjunCxZgs.getList({ current: 1, size: 9999 })
  ...
  // ==================== 8. 导出 ====================
  const exportColumns = (item: AnjunCxZgsData, index: number) => ({
  ...
  const handleExportCurrent = () => {
    const data = tableData.value as AnjunCxZgsData[]
    ...
    XLSX.utils.book_append_sheet(wb, ws, '案均赔款-支公司（车险）')
    XLSX.writeFile(wb, `案均赔款-支公司（车险）_${dateSuffix()}.xlsx`)
    ElNotification({ title: '成功', message: '导出成功', type: 'success' })
  }

  const handleExportAll = async () => {
    try {
      const res = await AnjunCxZgs.getList(tableApiParams.value)
      ...
      const wb = XLSX.utils.book_new()
      XLSX.utils.book_append_sheet(wb, ws, '案均赔款-支公司（车险）')
      XLSX.writeFile(wb, `案均赔款-支公司（车险）_全部_${dateSuffix()}.xlsx`)
