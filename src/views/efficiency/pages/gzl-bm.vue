  ...
  <div class="flex flex-col gap-2 pb-3">
    ...
    <ElCard class="flex-1 art-table-card" style="margin-top: 0;padding: 5px;">
      <template #header>
        <div class="flex-cb">
          <!-- 表格标题 + 动态统计时间 -->
          <h4 class="m-0">部门当日工作量【统计时间：{{ currentMaxTjTime }}】</h4>
...
<script setup lang="ts">
  ...
  import { GzlBm } from '@views/efficiency/api'
  ...
  const {
    ...
    core: {
      apiFn: async (params: UseTableParams): Promise<UseTableResult<DailyWorkloadBmData>> => {
        ...
        const response = await GzlBm.getList(queryParams)
  ...
  const handleRefresh = async () => {
    try {
      ...
      const res = await GzlBm.getList({ current: 1, size: 9999 })
  ...
  const handleExportCurrent = async () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, '部门当日工作量')
    const fileName = `部门当日工作量_${new Date().toLocaleDateString().replace(/\//g, '-')}.xlsx`
  ...
  const handleExportAll = async () => {
    try {
      const res = await GzlBm.getList(tableApiParams.value)
      ...
      const wb = XLSX.utils.book_new()
      XLSX.utils.book_append_sheet(wb, ws, '部门当日工作量')
      const fileName = `部门当日工作量_全部_${new Date().toLocaleDateString().replace(/\//g, '-')}.xlsx`
      XLSX.writeFile(wb, fileName)
