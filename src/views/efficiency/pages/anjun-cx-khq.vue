...
<script setup lang="ts">
  ...
  import { AnjunCxKhq } from '@views/efficiency/api'

  defineOptions({ name: 'AnjunCxKhqTable' })

  // ==================== 1. 类型定义 ====================
  interface AnjunCxKhqData {
  ...
  const { data: tableData, loading, error: tableError, pagination, refreshData, handleSizeChange, handleCurrentChange, columns, columnChecks } = useTable({
    core: {
      apiFn: async (params: UseTableParams): Promise<UseTableResult<AnjunCxKhqData>> => {
        ...
        const response = await AnjunCxKhq.getList(queryParams)
  ...
  const handleExportCurrent = () => {
    ...
    XLSX.writeFile(wb, `案均赔款-客户群（车险）_${dateSuffix()}.xlsx`)
    ElNotification({ title: '成功', message: '导出成功', type: 'success' })
  }

  const handleExportAll = async () => {
    try {
      const res = await AnjunCxKhq.getList(tableApiParams.value)
