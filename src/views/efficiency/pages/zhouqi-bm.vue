...
<script setup lang="ts">
  ...
  import { ZhouqiBm } from '@views/efficiency/api'
  ...
  const { data: tableData, loading, error: tableError, pagination, refreshData, handleSizeChange, handleCurrentChange, columns, columnChecks } = useTable({
    core: {
      apiFn: async (params: UseTableParams): Promise<UseTableResult<ZhouqiBmData>> => {
        ...
        const response = await ZhouqiBm.getList(queryParams)
  ...
  const handleRefresh = async () => {
    try {
      const res = await ZhouqiBm.getList({ current: 1, size: 9999 })
  ...
  const handleExportAll = async () => {
    try {
      const res = await ZhouqiBm.getList(tableApiParams.value)
