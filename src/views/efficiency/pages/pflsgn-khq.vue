...
<script setup lang="ts">
  ...
  import { PflsgnKhq } from '@views/efficiency/api'
  ...
  const { data: tableData, loading, error: tableError, pagination, refreshData, handleSizeChange, handleCurrentChange, columns, columnChecks } = useTable({
    core: {
      apiFn: async (params: UseTableParams): Promise<UseTableResult<PflsgnKhqData>> => {
        ...
        const response = await PflsgnKhq.getList(queryParams)
  ...
  const handleRefresh = async () => {
    try {
      const res = await PflsgnKhq.getList({ current: 1, size: 9999 })
  ...
  const handleExportAll = async () => {
    try {
      const res = await PflsgnKhq.getList(tableApiParams.value)
