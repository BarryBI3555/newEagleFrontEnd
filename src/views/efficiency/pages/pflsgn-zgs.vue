<template>
  <div class="flex flex-col gap-2 pb-3">
    ...
    <ElCard class="flex-1 art-table-card" style="margin-top: 0;padding: 5px;">
      <template #header>
        <div class="flex-cb">
          <h4 class="m-0">事故年赔付率-支公司【统计时间：{{ currentMaxTjTime }}】</h4>
...
<script setup lang="ts">
  ...
  import { PflsgnZgs } from '@views/efficiency/api'

  defineOptions({ name: 'PflsgnZgsTable' })

  // ==================== 1. 类型定义 ====================
  interface PflsgnZgsData {
  ...
  // ==================== 5. 构建下拉 ====================
  const buildDeptOptions = (data: PflsgnZgsData[]) => {
  ...
  const { data: tableData, loading, error: tableError, pagination, refreshData, handleSizeChange, handleCurrentChange, columns, columnChecks } = useTable({
    core: {
      apiFn: async (params: UseTableParams): Promise<UseTableResult<PflsgnZgsData>> => {
        const queryParams = {
          current: params.current, size: params.size,
          tjDate: tableApiParams.value.tjDate || '',
          comnameSgs: tableApiParams.value.comnameSgs ?? ''
        }
        const response = await PflsgnZgs.getList(queryParams)
  ...
  const handleRefresh = async () => {
    try {
      const res = await PflsgnZgs.getList({ current: 1, size: 9999 })
  ...
  // ==================== 8. 导出 ====================
  const exportColumns = (item: PflsgnZgsData, index: number) => ({
  ...
  const handleExportCurrent = () => {
    const data = tableData.value as PflsgnZgsData[]
    ...
    XLSX.utils.book_append_sheet(wb, ws, '事故年赔付率-支公司')
    XLSX.writeFile(wb, `事故年赔付率-支公司_${dateSuffix()}.xlsx`)
    ElNotification({ title: '成功', message: '导出成功', type: 'success' })
  }

  const handleExportAll = async () => {
    try {
      const res = await PflsgnZgs.getList(tableApiParams.value)
      ...
      const wb = XLSX.utils.book_new()
      XLSX.utils.book_append_sheet(wb, ws, '事故年赔付率-支公司')
      XLSX.writeFile(wb, `事故年赔付率-支公司_全部_${dateSuffix()}.xlsx`)
