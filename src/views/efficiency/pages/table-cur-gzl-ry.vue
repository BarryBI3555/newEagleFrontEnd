...
<script setup lang="ts">
  ...
  import { TableCurGzlRy } from '@views/efficiency/api'
  ...
  const {
    ...
    core: {
      ...
      apiFn: async (params: UseTableParams): Promise<UseTableResult<DailyWorkloadData>> => {
        ...
        const response = await TableCurGzlRy.getList(queryParams)
  ...
  const handleRefresh = async () => {
    try {
      ...
      const res = await TableCurGzlRy.getList(tableApiParams.value)      // axios 返回的已经是数据数组
  ...
  const handleExportAll = async () => {
    try {
      ...
      const res = await TableCurGzlRy.getList(tableApiParams.value)
