...
<script setup lang="ts">
  ...
  import { TableCurGzlGroup } from '@views/efficiency/api'
  ...
  const {
    ...
    core: {
      apiFn: async (params: UseTableParams): Promise<UseTableResult<DailyWorkloadGroupData>> => {
        ...
        const response = await TableCurGzlGroup.getList(queryParams)
  ...
  const handleRefresh = async () => {
    try {
      ...
      const res = await TableCurGzlGroup.getList({ current: 1, size: 9999 })
  ...
  const handleExportAll = async () => {
    try {
      ...
      const res = await TableCurGzlGroup.getList(tableApiParams.value)
