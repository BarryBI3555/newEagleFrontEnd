...
<script setup lang="ts">
  ...
  import { GzlRs } from '@views/efficiency/api'
  ...
  const {
    ...
    core: {
      apiFn: async (params: UseTableParams): Promise<UseTableResult<DailyWorkloadRsData>> => {
        ...
        // 最终接口
        const response = await GzlRs.getList(queryParams)
  ...
  const handleRefresh = async () => {
    try {
      ...
      const res = await GzlRs.getList({ size: 9999 })
  ...
  const handleExportAll = async () => {
    try {
      ...
      const res = await GzlRs.getList(tableApiParams.value)
