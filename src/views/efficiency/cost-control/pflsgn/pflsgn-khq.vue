...
<script setup lang="ts">
  ...
  import { axiosRequestPflsgnKhq } from '@/api/AllRequestMethods/index'
  ...
  const { data: tableData, loading, error: tableError, pagination, refreshData, handleSizeChange, handleCurrentChange, columns, columnChecks } = useTable({
    core: {
      apiFn: async (params: UseTableParams): Promise<UseTableResult<PflsgnKhqData>> => {
        ...
        const response = await axiosRequestPflsgnKhq(queryParams)
  ...
  const handleRefresh = async () => {
    try {
      const res = await axiosRequestPflsgnKhq({ current: 1, size: 9999 })
  ...
  const handleExportAll = async () => {
    try {
      const res = await axiosRequestPflsgnKhq(tableApiParams.value)
