...
<script setup lang="ts">
  ...
  import { axiosRequestPflsgnXny } from '@/api/AllRequestMethods/index'
  ...
  const { data: tableData, loading, error: tableError, pagination, refreshData, handleSizeChange, handleCurrentChange, columns, columnChecks } = useTable({
    core: {
      apiFn: async (params: UseTableParams): Promise<UseTableResult<PflsgnXnyData>> => {
        ...
        const response = await axiosRequestPflsgnXny(queryParams)
  ...
  const handleRefresh = async () => {
    try {
      const res = await axiosRequestPflsgnXny({ current: 1, size: 9999 })
  ...
  const handleExportAll = async () => {
    try {
      const res = await axiosRequestPflsgnXny(tableApiParams.value)
