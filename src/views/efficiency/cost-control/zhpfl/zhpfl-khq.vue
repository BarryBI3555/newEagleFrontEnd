...
<script setup lang="ts">
  ...
  import { axiosRequestZhpflKhq } from '@/api/AllRequestMethods/index'
  ...
  const { data: tableData, loading, error: tableError, pagination, refreshData, handleSizeChange, handleCurrentChange, columns, columnChecks } = useTable({
    core: {
      apiFn: async (params: UseTableParams): Promise<UseTableResult<ZhpflKhqData>> => {
        ...
        const response = await axiosRequestZhpflKhq(queryParams)
  ...
  const handleRefresh = async () => {
    try {
      const res = await axiosRequestZhpflKhq({ current: 1, size: 9999 })
  ...
  const handleExportAll = async () => {
    try {
      const res = await axiosRequestZhpflKhq(tableApiParams.value)
